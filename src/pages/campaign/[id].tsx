...
export default function CampaignPage() {
  ...
  const [campaignInfo, setCampaignInfo] = useState<any>({}); // State to store campaign info
  ...
  const initializeCountdown = (deadline: any) => {
    const deadlineDate = new Date(deadline * 1000); // Convert to milliseconds
    const now = new Date();
    setTimeLeft(
      Math.max(
        (deadlineDate as unknown as number) - (now as unknown as number),
        0,
      ),
    );
  };
  ...
  const formatTimeLeft = (milliseconds: any) => {
  ...
  const filteredContributors = (campaignInfo?.contributors || [])
  ...
  return (
    <Layout>
      {showAlert && (
        <div
          className="relative my-3 rounded border border-green-400 bg-green-100 px-4 py-3 text-green-700"
          role="alert"
        >
      ...
      <section>
        <article className="grid grid-cols-2 gap-8 p-8">
          ...
          <div className="flex flex-col gap-3">
            ...
            <p>
              You think crypto is turbolent? I invented AGI and still got fired!
              :(
            </p>
            ...
            <p>
              Enter the amount in Lamport. Remember 1 SOL = 10^9 (100000000)
              Lamport.
            </p>
          </div>
        </article>
        <div className="grid grid-cols-3 py-8">
          <div className="text-center">
            <h2 className="text-2xl">
              {campaignInfo.totalContributed / 1000000000} SOL
            </h2>
            <p>Raised</p>
          </div>
          ...
          <div className="text-center">
            <h2 className="text-2xl">
              {campaignInfo.threshold / 1000000000} SOL
            </h2>
            <p>Goal</p>
          </div>
        ...
        <div className="card mx-8 my-4 border-2 border-base-content bg-base-100 shadow-xl">
          ...
          >
            {images.map((image, index) => (
              <div
                key={index}
                className={`mx-2 w-60 flex-none ${
                  isFirstImageGreyedOut() && index === 0 ? "opacity-10" : ""
                }`}
              >
                <img
                  src={image.url}
